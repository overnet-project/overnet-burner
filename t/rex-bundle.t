...
is $bundle_a->{relative_dir}, 'artifacts/rex',
    'renderer reports run-local Rex bundle path';
is_deeply $bundle_a->{files},
    [
    'Rexfile',
    'actor-hosts.json',
...
is_deeply [map { $_->{actor_id} } @{ $assignments->{assignments} }],
    [
    qw(
        relay-001
        publisher-001
...
is_deeply [map { $_->{host_id} } @{ $assignments->{assignments} }],
    [('host-001') x 5],
...
is $relay_config->{env}{OVERNET_BURNER_ACTOR_ID}, 'relay-001',
    'per-actor config includes actor env';
is $relay_config->{env}{OVERNET_BURNER_ROLE}, 'relay',
    'per-actor config includes role env';
...
is_deeply [map { $_->{phase} } @{ $lifecycle->{commands} }],
...
is_deeply [map { $_->{actor_id} } @{ $collection->{metric_streams} }],
    [
    qw(
        relay-001
        publisher-001
...
like $rexfile, qr/^# Generated by overnet-burner Rex bundle renderer\./,
    'Rexfile is clearly generated';
like $rexfile, qr/task 'bootstrap'/, 'Rexfile has bootstrap task stub';
like $rexfile, qr/task 'cleanup'/, 'Rexfile has cleanup task stub';
like $rexfile, qr/task 'bootstrap', sub \{/,
    'Rexfile renders lifecycle tasks as local Rex tasks';
unlike $rexfile, qr/task 'bootstrap', group => 'all'/,
    'Rexfile lifecycle tasks do not force SSH to localhost';
unlike $rexfile, qr/--provider rex/, 'Rexfile does not treat Rex as a provider';
...
my $rendered_chaos = _read_json(
    File::Spec->catfile($chaos_dir, 'artifacts', 'rex', 'chaos-hooks.json'),
);
is $rendered_chaos->{hooks}[0]{id}, 'chaos-001',
    'renders chaos hook id from plan';
is $rendered_chaos->{hooks}[0]{at_seconds}, 30,
    'renders chaos hook schedule from plan';
is $rendered_chaos->{hooks}[0]{action}, 'restart',
    'renders chaos hook action from plan';
